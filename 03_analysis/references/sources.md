...
|---|---|---|---|
| C01 | `01_official/originals/[붙임1] 공고문.pdf` 및 `01_official/text/붙임1_공고문.txt` | 사용 | 지정공모 3, 가점 3점, 평가항목, 자격, 일정, 제출물, 선행사업·수상작 |
| C02 | `01_official/originals/[붙임2] 신청양식.hwp` 및 `01_official/text/붙임2_신청양식.txt` | 사용 | 서식 1~5, 제안서 10쪽 이내, 보고서 권장 3~5쪽, AI 기록 필드 |
| C03 | `01_official/originals/[붙임3] 신청가이드.pdf` 및 `01_official/text/붙임3_신청가이드.txt` | 사용 | 참가·제출 절차 요약 |
| C04 | `01_official/originals/[붙임4] PMS신청매뉴얼.pdf` 및 `01_official/text/붙임4_PMS매뉴얼.txt` | 사용 | PMS 입력·파일 업로드·임시저장·접수 절차 |
...
| 산출물 | 직접 입력 | 처리 |
|---|---|---|
| `03_analysis/data/urban_events.csv` | D01 | 공사 이벤트를 사람이 전사하고 확인시각·상태를 기록 |
| `03_analysis/results/traffic_quality.json` | D01의 1·12공구 과거 관측 | 11스냅숏·451행·41구간의 고정 품질요약; 원응답과 SQLite는 이 작업폴더에 없음 |
| `03_analysis/data/standard_corridor_evidence.csv` | D06·D07 | EPSG:5186 영역 필터 후 공식 링크 길이 방향성 최단경로 |
| `03_analysis/data/event_scope_evidence.csv` | D01의 공식 이미지 2건·D09의 공지문 | URL·기대문구·이미지 SHA-256과 공사범위 신뢰도를 기록 |
| `03_analysis/results/mapping_evidence_validation.json` | `event_scope_evidence.csv`의 공식 URL·자산 | 당시 확인한 HTTP 상태·문구·PNG 규격·해시 기록 |
| `03_analysis/data/event_segment_mapping.csv` | 공사 이벤트·현재 교통라벨·표준 링크·공식 범위근거 | 사람이 포함/후보/제외와 범위/개별라벨 신뢰도를 구분해 판정 |
| `03_analysis/data/event_exposure_geometry.csv` | D01·D06·D07의 공식 공사범위와 표준 링크/노드 | 이벤트별 점·링크 참조, 250m 반경, 공간근거 신뢰도를 고정 |
| `03_analysis/results/exposure_validation.json` | D10 대전 CSV + `event_exposure_geometry.csv` | WGS84→EPSG:5186 변환, 점–이벤트 거리, 원본 품질·해시·이벤트별 상가 수 기록 |
| `03_analysis/results/segment_exposure.csv` | 이벤트별 상가 수 + `event_segment_mapping.csv` | 시범 관측구간 10개에 노출값·단위·기준일·공간/매핑 신뢰도를 연결 |
| `03_analysis/results/current_risk.csv` | 마지막 관측 + 검증된 시범구간 노출값 | 현재 속도 기반 관측등급과 노출 대리값을 별도 필드로 제공; 예측필드는 null |
| `03_analysis/results/forecast_readiness.json` | 과거 관측 품질요약 | 최소자료 미달을 `insufficient_data`로 고정하고 AI 예측 성능 주장을 잠금 |
...
- 수집 원응답과 SQLite: 이 정리본에는 포함하지 않으며, 제출 근거는 위 고정 요약과 검증표로 한정
